% =========================================================================
% Flow-Population Allometric Scaling 
% Using Flow-Derived Structural Regimes
%
% Description:
% This script investigates the allometric scaling relationship between urban 
% flows and population distributions. Instead of deriving new crossover points, 
% it directly projects the scaling regimes previously identified from flow 
% scaling onto the flow-population phase space. 
% =========================================================================

% --- Setup and Empirical Data Extraction ---
% Extract dimensions from the cleaned datasets
[m, ~] = size(T_cleaned);
[~, n] = size(x_final);
n = n + 2;

% Isolate independent (Population) and dependent (Flow) variables
% x: Empirical population distribution matrix
% y: Empirical flow distribution matrix
x = table2array(P_cleaned(:, 3 : n));
y = table2array(T_cleaned(:, 3 : n)); 

% Define output table variable names to capture comprehensive regression statistics
% for the allometric scaling models dictated by flow-derived structures
...
% Initialize standardized arrays for spatial alignment
x1_final = nan(m, n);
y_final = nan(m, n);

% Filter out missing observations (NaNs) to ensure strict spatial alignment 
% between population (x) and flow (y) sequences across all urban samples
for i = 1 : m
    non_empty_indices = find(~isnan(x_final(i, :)));
    x1_final(i,1:length(non_empty_indices)) = x(i, non_empty_indices);
    y_final(i,1:length(non_empty_indices)) = y(i, non_empty_indices);
end

% =========================================================================
% Main Processing Loop: Evaluating Regime-Specific Allometry
% =========================================================================
for i = 1 : m
    
    % Verify if the current sample has a valid flow-derived model classification 
    % (stored in the reference table T from the previous analysis)
    if any(T.sampleID == i)
        
        rowIndices = find(T.sampleID == i);
        current_type = char(T.type(rowIndices)); % Flow-derived optimal model type
        
        % Isolate valid spatial sequences and compute cumulative distributions
        valid_idx = ~isnan(y_final(i,:));
        x_full = cumsum(x1_final(i,valid_idx)');
        ...
        n_samples = length(x_current);  % Define the effective sample size for the current city     
        ...
        % -----------------------------------------------------------------
        % (1) Baseline Global Allometry
        % Evaluates the macroscopic allometric scaling assuming a single, invariant 
        % exponent.
        % -----------------------------------------------------------------
        if strcmp(string(current_type), 'global')
            ...
            resultTable.global_slope(i) = beta(2); % Allometric scaling exponent
            resultTable.global_intercept(i) = beta(1); % Scaling prefactor
            resultTable.global_r2(i) = stats(1);
            resultTable.global_F(i) = stats(2);
            resultTable.global_pValue(i) = stats(3);

        % -----------------------------------------------------------------
        % (2) Segmented Allometry (Flow-Dictated Breakpoints)
        % Applies the exact crossover points identified from flow scaling 
        % to partition the flow-population phase space.
        % -----------------------------------------------------------------
        elseif strcmp(string(current_type), 'segmented')
            % Retrieve the optimal segment lengths dictated by flow scaling
            ...
            % Ensure reconstructed breakpoints do not exceed empirical data bounds
            bp = bp(bp < n_samples);
            
            % Define the topological boundaries for each allometric scaling regime
            segment_edges = [1, bp+1, n_samples+1];
            num_segments = length(segment_edges)-1;
            
            % Initialize cell arrays to store regime-specific allometric parameters
            ...
            for s = 1:num_segments
                ...
                % Extract regime-specific data and transform to log-log phase space
                ...
                % Store regime-specific allometric scaling exponents and fit statistics
                seg_slope{s} = betas(2);
        ...
        % -----------------------------------------------------------------
        % (3) Trimmed Allometric Regression (Flow-Dictated Trimming)
        % Fits a robust allometric model by removing the exact boundary observations 
        % (tails) that were identified as spatial noise or edge effects in the 
        % flow scaling analysis.
        % -----------------------------------------------------------------
        elseif strcmp(string(current_type), 's-global')
            ...
            % Calculate the effective index range after trimming spatial edge effects
            idx1 = bp1+1;
            idx2 = n_samples-bp2;
            ...
            % Perform OLS regression on the trimmed core dataset
            [beta, ~, ~, ~, stats] = regress(log_y, X);
